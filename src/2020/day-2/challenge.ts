...
const newIsValidPw = ([min, max, letter, word]: (string | number)[]): boolean => {
    const w = `${word}`;
    const pos1 = w[+min - 1];
    const pos2 = w[+max - 1];
...
const validPasswords = (validatePasswords:boolean[]): boolean[] => filter((isValid) => isValid === true, validatePasswords);

const processPasswords = (listOfPasswords: (string | number)[][]): boolean[] => {
    const validatePasswords = listOfPasswords.map(pass => isValidPw(pass));
    return validPasswords(validatePasswords);
}

const newProcessPassword = (listOfPasswords: (string | number)[][]): boolean[] => {
